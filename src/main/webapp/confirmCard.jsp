...
<body style="background-color: #E3D7A3;">
	...
	<h3>
		<p style="color: brown">下記の内容で注文を行いますか？</p>
	</h3>
	<h3>
		<p style="color: brown">ご注文商品</p>
	</h3>
	...
	<c:if test="${not empty cart.items}">
		...
		<form action="/shopping/OrderServlet?action=orderCard" method="post">
			...
			<h3>
				<p style="color: brown">支払方法</p>
			</h3>
			${message} <br> クレジットカードの名義：${card.name} <br> クレジットカード番号：
			${card.num} <br> <br> 有効期限：月${card.month}/年${card.year} <br>
			<br> セキュリティコード：${card.pass} <br> <br> <input
